...
class GamesTable(tag: Tag) extends Table[Game](tag, "games") {
  ...
  def author_solution = foreignKey("AUTHOR_SOLUTION", author_solutionID, TableQuery[SolutionsTable])(.id)

  def dumb_solutionID = column[Long]("dumb_solutionID")

  def dumb_solution = foreignKey("DUMB_SOLUTION", dumb_solutionID, TableQuery[SolutionsTable])(.id)
